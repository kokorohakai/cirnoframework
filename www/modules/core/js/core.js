...
enyo.kind({
    ...
    classes: [(Cirno.mobile)?"mobile":"nonmobile"],
    constructor: function( ){
        ...
        var self = this;
        var module = {};
        var temp = window.module.split("/");
        for ( var i = 0, len = temp.length; i < len; i++ ){
            temp[i] = temp[i].ucfirst();
        }
        
        module.kind = "Cirno."+temp.join(".");
        module.path = window.module;

        var ajax = new enyo.Ajax({
            url:"/api/"+window.module,
            method:"GETCONFIG"
